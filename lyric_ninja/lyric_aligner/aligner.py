...
@njit
def _get_trellis_np_jit(
    emission: np.ndarray, tokens: List[int], blank_id: int
) -> np.ndarray:
...
@timer
def get_trellis_np(
    emission: np.ndarray, tokens: List[int], blank_id: int = 0
) -> np.ndarray:
    return _get_trellis_np_jit(emission.astype(np.float32), tokens, blank_id)


@njit
def _backtrack_np_jit(
    trellis: np.ndarray, emission: np.ndarray, tokens: List[int], blank_id: int
...
@timer
def backtrack_np(
    trellis: np.ndarray, emission: np.ndarray, tokens: List[int], blank_id: int = 0
) -> List[Point]:
    raw_path = _backtrack_np_jit(
        trellis.astype(np.float32), emission.astype(np.float32), tokens, blank_id
    )
    return [
        Point(token_index, time_index, score)
        for token_index, time_index, score in raw_path
    ]
...
class LyricAligner:
    ...
    @timer
    def _get_emissions(
        self, waveform: Union[torch.Tensor, np.ndarray], sr: int
    ) -> np.ndarray:
    ...
    @timer
    def get_timed_words(
        self, waveform: Union[np.ndarray, torch.Tensor], transcript: str, sr: int
    ) -> List[Dict[str, Any]]:
        try:
            ...
            timed_words = [
                {
                    "text": word.label,
                    "start": (word.start * ratio) / sr,
                    "end": (word.end * ratio) / sr,
                }
        ...
        try:
            ...
            if file_ext == ".mp3":
                ...
                if audio.tags is None:
                    audio.add_tags()
                audio.tags.add(
                    USLT(encoding=Encoding.UTF8, lang="eng", desc="", text=lrc_content)
                )
                audio.save()
    ...
    def create_lrc_content(
        self, lyric_lines: List[Dict[str, Any]], metadata: Dict[str, str]
    ) -> str:
    ...
    def process_song(
        ...
        try:
            ...
            original_lines = [
                line.strip()
                for line in original_lyrics_text.split("\n")
                if line.strip()
            ]
            ...
            for line in original_lines:
                ...
                line_words = [
                    "".join(c for c in word if c in self.dictionary) for word in words
                ]
                line_words = [w for w in line_words if w]
                if line_words:
                    lines_with_words.append(
                        {"original_line": line, "words": line_words}
                    )
            ...
            if not transcript:
                print(
                    f"Could not find any valid characters for alignment in {lyric_file}"
                )
                return None
            ...
            for line_info in lines_with_words:
                ...
                if timed_word_cursor + num_words > len(timed_words):
                    print(
                        f"Warning: Not enough timed words for line: {line_info['original_line']}"
                    )
                    break
                start_time = timed_words[timed_word_cursor]["start"]
                end_time = timed_words[timed_word_cursor + num_words - 1]["end"]
                final_lyric_lines.append(
                    {
                        "start": start_time,
                        "end": end_time,
                        "text": line_info["original_line"],
                    }
                )
            ...
            if create_lrc:
                lrc_file_path = self.create_lrc_file(
                    audio_name=audio_name,
                    lyric_lines=final_lyric_lines,
                    metadata=metadata,
                )
                print(f"\n✓ LRC file: {lrc_file_path}")
            if embed_lyrics:
                synced_file_path = self.embed_lyrics_to_audio(
                    audio_file, final_lyric_lines, metadata
                )
                if synced_file_path:
