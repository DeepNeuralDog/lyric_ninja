...
class LyricAligner:
    def __init__(
        ...
        # self.separator = Separator(output_dir=self.sep_path, mdx_params=mdx_params, sample_rate=self.bundle.sample_rate)
        # self.sep_model_filename = sep_model_filename
        # if sep_model_filename:
        #     self.separator.load_model(model_filename=sep_model_filename)
    ...
    def _wav2vec2_inference(self, waveform_np: np.ndarray) -> torch.Tensor:
        if self.use_coreml:
            ...
            for i in range(0, total_samples, chunk_size):
                ...
                input_dict = {"waveform": chunk}
                emissions_chunk = self.model.predict(input_dict)[output_key]
                all_emissions.append(emissions_chunk)
            return torch.from_numpy(np.concatenate(all_emissions, axis=1))
        else:
    ...
    def wav2vec2_inference(self, waveform_np: np.ndarray) -> torch.Tensor:
        if self.use_coreml:
            ...
            return torch.from_numpy(all_emissions)
        else:
            waveform = torch.from_numpy(waveform_np).to(self.device)
            with torch.no_grad():
                emissions, _ = self.model(waveform)
    ...
    def get_timed_words(self, audio_file: str, transcript: str) -> List[Dict[str, Any]]:
        try:
            waveform, _ = librosa.load(audio_file, sr=self.bundle.sample_rate, mono=True)
            ...
            emission_np = log_probs[0].cpu().numpy()
            ...
            ratio = waveform_np.shape[1] / emission_np.shape[0]
            timed_words = [
